...
class NQLearning(LearningStrategy):
    ...
    def __init__(self, steps: int, env: Environment, learning_rate=0.01, reward_discount_rate=0.95, decay_rate=0.001,
                 epsilon=1, epsilon_min=0.01, epsilon_max=1.0):
    ...
    def evaluate(self, percept: Percept):
        ...
        # Add in front because we want to update first the most recent Q value!
        # Because the preceding Q values can make use of the new calculated Q value in the step before.
        self.mdp.update(percept)

        if len(self._percept_n_history) >= self._steps:
            for p in self._percept_n_history:
                self._update_q(p)
            self._percept_n_history.pop()
